...
scene("menu", () => {
    addButton("Play", vec2(width() / 2, 300), 3, () => {go("levels"); play("click")});
    addButton("Backpack", vec2(width() / 2, 370), 3, () => {go("backpack");play("click")});
    addButton("Quit", vec2(width() / 2, 440), 3, () => play("click"));
...
scene("levels", (page) => {
    for(let i = 0; i < levels.length; i++) {
        if(i < 5) {
            addLevelBox(i + 1, vec2(370 + 110 * i, 250), 3, () => {go("main", i);play("click")});
        }
        else {
            addLevelBox(i + 1, vec2(370 + 110 * (i - 5), 360), 3, () => {go("main", i);play("click")});
        }
    }
...
scene("instructions", () => {
    addButton("Daun?", vec2(width() / 2, 300), 3, () => {go("menu");play("click")});
...
function pathSearchGhost(player, enemy) {
    if(player.pos.x > enemy.pos.x && player.pos.y > enemy.pos.y) {
        enemy.move(110, 110);
